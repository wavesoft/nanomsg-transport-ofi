...
NANOMSG_TRANSPORT_OFI="${NANOMSG_DIR}/src/transports/ofi"
[ -L "${NANOMSG_TRANSPORT_OFI}" ] && echo -e "** FAILED **\nOFI Transport seems to be installed already!" && exit 1
[ -d "${NANOMSG_TRANSPORT_OFI}" ] && echo -e "** FAILED **\nOFI Transport seems to be installed by another source!" && exit 1
...
echo -n "Link: "
ln -v -s ${CURR_PATH}/src/transports/ofi ${NANOMSG_TRANSPORT_OFI}
[ $? -ne 0 ] && echo -e "** FAILED **\nUnable to create symbolic link!" && exit 1
